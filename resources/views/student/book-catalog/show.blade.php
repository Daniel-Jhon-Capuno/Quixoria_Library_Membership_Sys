            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 mb-4">
                ...
                @else
                    <div class="bg-slate-800 border-l-4 border-slate-700 p-3 text-sm text-gray-200 rounded">
                        ...
                        <a href="{{ route('student.subscription.index') }}" class="ml-1 text-gray-100 underline">Subscribe</a>
                    </div>
                @endif
            </div>
        @endif
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="lg:col-span-1">
                    <div class="bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6">
                            <!-- Cover Image -->
                            <div class="aspect-w-3 aspect-h-4 bg-slate-700 mb-6">
                                @if($book->cover_image)
                            ...
                            <div class="mb-6">
                                <div class="flex items-center justify-between mb-2">
                                    <span class="text-sm font-medium text-gray-200">Availability:</span>
                                    ...
                                    <p class="text-sm text-gray-300">Total copies: {{ $book->total_copies }}</p>
                            </div>
                            ...
                            <div class="space-y-3">
                                ...
                                @if($atWeeklyLimit)
                                    <div class="w-full px-4 py-3 bg-slate-800 border-l-4 border-slate-700 text-sm text-gray-200 rounded">
                                ...
                                @else
                                    @if(!$atWeeklyLimit && $canBorrow && $availableCopies > 0)
                                        <form method="POST" action="{{ route('student.borrow-requests.store', $book->id) }}">
                                            @csrf
                                            <button type="submit" data-confirm="Are you sure you want to borrow this book?" class="w-full inline-flex justify-center items-center px-4 py-2 bg-cyan-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-cyan-500">
                                                Borrow This Book
                                            </button>
                                        </form>
                                    @elseif(!$canBorrow)
                                    ...
                                    @if($canReserve && !$hasReservation && $availableCopies == 0)
                                        <form method="POST" action="{{ route('student.reservations.store') }}">
                                            ...
                                            <button type="submit" class="w-full inline-flex justify-center items-center px-4 py-2 bg-cyan-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-cyan-500">
                                                Reserve This Book
                                            </button>
                                        </form>
                                    @elseif($hasReservation)
                                        <div class="w-full px-4 py-2 bg-yellow-800 border border-yellow-700 rounded-md text-sm font-medium text-yellow-100 text-center">
                                            Already Reserved
                                        </div>
                                    @elseif(!$canReserve)
                                        <div class="w-full px-4 py-2 bg-slate-700 border border-transparent rounded-md text-sm font-medium text-gray-300 text-center cursor-not-allowed">
                            ...
                            <!-- Back Button -->
                            <div class="mt-6">
                                <a href="{{ route('student.book-catalog.index') }}" class="inline-flex items-center text-sm text-gray-400 hover:text-gray-100">
                                    ← Back to Catalog
                                </a>
                            </div>
                        </div>
                ...
                <div class="lg:col-span-2">
                    <div class="bg-slate-800 overflow-hidden shadow-sm sm:rounded-lg">
                        <div class="p-6">
                            <h3 class="text-2xl font-bold text-gray-100 mb-2">{{ $book->title }}</h3>
                            <p class="text-lg text-gray-300 mb-6">by {{ $book->author }}</p>

                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                @if($book->isbn)
                                    <div>
                                        <label class="block text-sm font-medium text-gray-200">ISBN</label>
                                        <p class="mt-1 text-sm text-gray-100">{{ $book->isbn }}</p>
                                    </div>
                                @endif

                                @if($book->genre)
                                    <div>
                                        <label class="block text-sm font-medium text-gray-200">Genre</label>
                                        <p class="mt-1 text-sm text-gray-100">{{ $book->genre }}</p>
                                    </div>
                                @endif
                                ...
                                @if($book->category)
                                    <div>
                                        ...
                                        <p class="mt-1 text-sm text-gray-100">{{ $book->category }}</p>
                                    </div>
                                @endif

                                @if($book->publication_year)
                                    <div>
                                        <label class="block text-sm font-medium text-gray-200">Publication Year</label>
                                        <p class="mt-1 text-sm text-gray-100">{{ $book->publication_year }}</p>
                                    </div>
                                @endif
                            </div>
                            ...
                            @if($book->description)
                                <div class="mb-6">
                                    ...
                                    <div class="text-sm text-gray-100 leading-relaxed">
                            ...
                            <div class="border-t pt-6">
                                <h4 class="text-lg font-medium text-gray-100 mb-4">Additional Information</h4>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div class="bg-slate-700 p-4 rounded-lg">
                                        <h5 class="font-medium text-gray-100 mb-2">Borrowing Information</h5>
                                    ...
                                    @if(auth()->check())
                                        ...
                                        <div class="bg-slate-800 p-4 rounded-lg">
                                            <h5 class="font-medium text-gray-100 mb-2">Your Status</h5>
                                            <ul class="text-sm text-gray-400 space-y-1">
                                                ...
                                                @endif
                                                        ...
                                                        @if(isset($weeklyBorrows, $tierLimit))
                                                                    <li class="text-sm text-gray-400">{{ $weeklyBorrows }} of {{ $tierLimit }} used this week</li>
                                                        @endif
