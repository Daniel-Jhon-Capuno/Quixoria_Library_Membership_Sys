<x-app-layout>
    ...
    @if(($overdueBorrows ?? collect())->count() > 0)
        <div class="mb-6 rounded-xl p-6 flex items-start gap-4 bg-red-800/20 border border-red-700">
            ...
            <div>
                ...
                <p class="text-sm mt-1 text-gray-400">You have overdue items. Please return them as soon as possible to avoid late fees and maintain your borrowing privileges.</p>
            </div>
        </div>
    @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <!-- Subscription Card -->
        <div class="lg:col-span-2 rounded-xl p-6 shadow-card bg-slate-800 border border-slate-700">
                ...
                <div class="space-y-4">
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-gray-100 mt-1">{{ $subscription->membershipTier->name ?? 'N/A' }}</p>
                        </div>
                    ...
                    <div style="border-top-color: rgb(var(--border-primary));" class="border-t pt-4">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <p class="text-lg font-bold mt-1 text-gray-100">{{ $subscription->membershipTier->borrow_limit_per_week ?? 0 }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-400">Borrow Period</p>
                                <p class="text-lg font-bold mt-1 text-gray-100">{{ $subscription->membershipTier->borrow_duration ?? 14 }} days</p>
                            </div>
                        </div>
                    </div>
                    <div style="border-top-color: rgb(var(--border-primary));" class="border-t pt-4">
                        <p class="text-xs text-gray-400">Expires</p>
                        <p class="font-semibold mt-1 text-gray-100">{{ $subscription->ends_at->format('M d, Y') ?? 'N/A' }}</p>
                        <p class="text-xs mt-1 text-gray-400">{{ $subscription->ends_at->diffForHumans() ?? 'N/A' }}</p>
                    </div>
                </div>
            @else
                <div class="text-center py-6">
                    <p class="mb-4" style="color: rgb(var(--text-secondary));">You don't have an active subscription</p>
                    <a href="#" class="inline-block px-4 py-2 bg-primary hover:bg-primary-dark rounded-lg text-white font-medium transition">
                        Get Subscription
                    </a>
                </div>
            @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div class="rounded-xl p-6 shadow-card"
                ...
                @else
                    <div class="text-center py-8" style="color: rgb(var(--text-secondary));">
                        ...
                        <a href="#" class="mt-3 text-primary hover:text-primary-dark transition font-medium">Browse our collection →</a>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="space-y-4">
            ...
            <!-- Actions -->
            <div class="space-y-2">
                <a href="#" class="block w-full px-4 py-3 rounded-lg text-white font-medium transition text-center bg-primary hover:bg-primary-dark">
                    Browse Books
                </a>
                <a href="#" class="block w-full px-4 py-3 rounded-lg font-medium transition text-center border border-accent text-accent bg-accent/20 hover:bg-accent/30">
                    View Profile
                </a>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            @forelse($recommendedBooks ?? [] as $book)
                <div class="rounded-lg p-4 cursor-pointer group transition"
                    ...
                    <button class="w-full mt-3 px-3 py-2 rounded text-xs font-medium transition bg-primary/20 hover:bg-primary/30 text-primary">
                        + Borrow
                    </button>
                </div>
            @empty
